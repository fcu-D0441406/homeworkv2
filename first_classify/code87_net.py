...
class Resnext:
    ...
    def build_net(self,trainable):
        with tf.variable_scope('resnext',reuse=tf.AUTO_REUSE):
            ...
            resnext2_1 = self.batch_relu(tf.layers.conv2d(self.resnext2,256,1,1,padding='SAME'),trainable)
            resnext2_2 = self.batch_relu(tf.layers.conv2d(self.resnext2,256,3,1,padding='SAME'),trainable)
            resnext2_ct = self.Concatenation([resnext2_1,resnext2_2])

...
class ResNet():#resnet34(3,4,6,3) resnet18(2,2,2,2)
    ...
    def unsample(self,scope='upresnet'):
        ...
        with tf.variable_scope(scope):
            ...
            print(self.unconv1)
            self.unconv2 = tf.layers.conv2d_transpose(self.unconv1,128,3,2,padding='SAME')
            print(self.unconv2)
            self.unconv3 = tf.layers.conv2d_transpose(self.unconv2,64,3,2,padding='SAME')
            print(self.unconv3)
            self.unconv4 = tf.layers.conv2d_transpose(self.unconv3,64,3,2,padding='SAME')
            print(self.unconv4)
            self.unconv5 = tf.layers.conv2d_transpose(self.unconv4,self.class_num,3,2,padding='SAME')
            #self.unconv5 = tf.reshape(self.unconv5,[-1,img_size*img_size*2])
            print(self.unconv5)

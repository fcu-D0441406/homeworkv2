    ...
    for i in range(bf_check.shape[0]):
        for j in range(bf_check.shape[1]):
            for a in range(ratio_num):
                for aa in range(len(anchor_box[p])):
                    #if(bf_check[i][j][a][0]>=0.0):
                    #print('hi')
                    height = anchor_box[p][a]*ratio[a][0]
                    width = anchor_box[p]*ratio[a][1]
                    c_x = (j+1)*anchor_box[p]*ratio[a][1]+width*loc[i][j][a][0]
                    c_y = (i+1)*anchor_box[p]*ratio[a][0]+height*loc[i][j][a][1]
                    #print(loc[i][j][a*4+2])
                    c_width = width*float(np.exp(loc[i][j][a][2]))
                    c_height = height*float(np.exp(loc[i][j][a][3]))
...
def rpn_fg_loss(fg,r_fg,fg_batch,p):
    test_array = np.zeros([fg.shape[0],fg.shape[1],3])
    pos = 0
    neg = 0
                ...
                if(r_fg[i][j][a][0]==1):
                    for b in range(r_box.shape[3]):
                        if(r_box[i][j][a][b]==1):
                            ...
                            p_x,p_y,p_w,p_h = j*anchor_box[p]*ratio[a][1],i*anchor_box[p]*ratio[a][0],anchor_box[p]*ratio[a][1],anchor_box[p]*ratio[a][0]
...
def train_rpn(net,label_loc):
    ...
    real_fg0 = tf.placeholder(tf.float32,[None,7,7,3,2])
    real_test_fg0 = tf.placeholder(tf.float32,[None,7,7,3])
    
    
    real_fg1 = tf.placeholder(tf.float32,[None,14,14,3,2])
    real_test_fg1 = tf.placeholder(tf.float32,[None,14,14,3])
    
    real_fg2 = tf.placeholder(tf.float32,[None,28,28,3,2])
    real_test_fg2 = tf.placeholder(tf.float32,[None,28,28,3])
    
    real_fg3 = tf.placeholder(tf.float32,[None,56,56,3,2])
    real_test_fg3 = tf.placeholder(tf.float32,[None,56,56,3])
    
    
    bounding_box0 = tf.placeholder(tf.float32,[None,7,7,3,4])
    bounding_box1 = tf.placeholder(tf.float32,[None,14,14,3,4])
    bounding_box2 = tf.placeholder(tf.float32,[None,28,28,3,4])
    bounding_box3 = tf.placeholder(tf.float32,[None,56,56,3,4])
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            bs_batch,t_array2 = rpn_fg_loss(sfg2,r_fg2[0],bs_batch,0.3)
            bs_batch,t_array3 = rpn_fg_loss(sfg3,r_fg3[0],bs_batch,0.15)
            ...
            if(bx_batch==0):
                bx_batch = 1
            '''
            ls,_ = sess.run([loss,train_op],feed_dict={x:input_x,
            ...
            '''
            '''
            ls,_ = sess.run([box_loss,train_op],feed_dict={x:input_x,
